...
import './App.css'
import { languageOptions, siteContent, type Language } from './domain/siteContent'
...
const getServiceIllustration = (slug: string) => {
  switch (slug) {
    case 'septic-tank-cleaning':
      return createIllustration('Septic Cleaning', '🧼', '#b42318')
    case 'septic-system-installation':
      return createIllustration('System Installation', '🛠️', '#8f2a24')
    case 'septic-tank-maintenance':
      return createIllustration('Maintenance Plan', '🧰', '#a3412f')
    case 'septic-system-repair':
      return createIllustration('System Repair', '🔧', '#b42318')
    case 'septic-tank-pumping':
      return createIllustration('Tank Pumping', '💧', '#7a4a4a')
    case 'grease-trap-cleaning':
      return createIllustration('Grease Trap Care', '🍳', '#a3412f')
    case 'grease-trap-pumping':
      return createIllustration('Grease Trap Pumping', '🛁', '#8f2a24')
    case 'air-pump-installation':
      return createIllustration('Air Pump Setup', '🌬️', '#b42318')
    case 'air-pump-repair':
      return createIllustration('Air Pump Repair', '⚙️', '#7a4a4a')
    case 'air-compressor-installation':
      return createIllustration('Compressor Setup', '📦', '#8f2a24')
    case 'air-compressor-repair':
      return createIllustration('Compressor Repair', '🔩', '#a3412f')
    case 'septic-system-drain-out':
      return createIllustration('Drain-Out Service', '🚰', '#b42318')
    case 'emergency-septic-services':
      return createIllustration('Emergency Support', '🚑', '#8f2a24')
    default:
      return createIllustration('Septic Service', '🧽', '#b42318')
  }
}
...
const mapEmbedUrl = 'https://www.google.com/maps?q=Tyler,TX|Flint,TX|Frankston,TX|Chandler,TX|Lindale,TX|Mineola,TX|Winona,TX|Longview,TX|Canton,TX|Athens,TX|Henderson,TX|Overton,TX|Kilgore,TX|Grand+Saline,TX&output=embed&markers=size:mid%7Ccolor:red%7Clabel:S'
    ...
    <div className="app-shell">
      ...
      <main className="main-content">
        <Routes>
          ...
          />
          <Route path="/services/:slug" element={service ? <section className="section service-detail-section"><Link className="back-link" to="/services">← {language === 'en' ? 'Back to services' : 'Volver a servicios'}</Link><div className="service-detail-card"><p className="eyebrow">{language === 'en' ? 'Service detail' : 'Detalle del servicio'}</p><h2>{service.detailTitle}</h2><p>{service.detailDescription}</p><ul>{service.highlights.map((highlight) => <li key={highlight}>{highlight}</li>)}</ul></div></section> : <section className="section"><p>{language === 'en' ? 'Service not found.' : 'Servicio no encontrado.'}</p></section>} />
          ...
          <Route path="/area" element={<section className="section" id="area"><div className="section-heading"><p className="eyebrow">{content.nav[7].label}</p><h2>{content.nav[7].label}</h2></div><div className="area-layout"><div className="info-card area-summary-card"><h3>{language === 'en' ? 'Service areas' : 'Áreas de servicio'}</h3><ul className="area-list">{content.area.map((item) => <li key={item.title}><div className="area-list-item"><span className="area-marker" aria-hidden="true"><svg viewBox="0 0 64 64" role="img"><rect x="12" y="18" width="40" height="28" rx="6" fill="#b42318" /><rect x="22" y="12" width="20" height="10" rx="4" fill="#7a4a4a" /><rect x="28" y="24" width="8" height="14" rx="2" fill="#fff" /></svg></span><div className="area-list-content"><span>{item.title}</span><p>{item.description}</p></div></div></li>)}</ul></div><div className="map-card"><iframe title="Service area map" src={mapEmbedUrl} loading="lazy" referrerPolicy="no-referrer-when-downgrade" allowFullScreen /></div></div></section>} />
